...
int main() {
  ...
  int lane = 1;           //The start lane is defaulted by "1". 
  double velocity_ref = 0.0;  //The start velocity is set to 0.

  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,
               &map_waypoints_dx,&map_waypoints_dy, &lane, &velocity_ref]
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          for(int i = 0; i < sensor_fusion.size(); i++){
            float target = sensor_fusion[i][6];
            int check_lane = -1;
            ...
            //check the speed and distance of other cars
            ...
            if(check_lane - lane == 0){
              isCarAhead |= check_car_s > car_s && check_car_s - car_s < 30;
            }else if(check_lane - lane == -1){
              isCarOnLeft |= car_s - check_car_s < 20 && check_car_s - car_s < 20;
            }else if(check_lane - lane == 1){
              isCarOnRight |= car_s - check_car_s < 20 && check_car_s - car_s < 20;
            }
          }
          
          //Behavior Planning
          //If another car is in front of me, I should change my lane or speed down. If there is no car ahead, I should speed up. 
          double velocity_adj = 0.0;
          ...
          if(isCarAhead){
            if(!isCarOnLeft && lane != 0){
              ...
              velocity_adj -= MAX_ACC;
            }
          }else{
            if(velocity_ref < MAX_SPEED){
              velocity_adj += MAX_ACC;
            }
          }
          
          //Trajectory Generation by Using Spline
          vector<double> ptsx;
          vector<double> ptsy;
          double ref_x = car_x;
          double ref_y = car_y;
          ...
          for(int i = 0; i <= 50 - prev_size; i++){
            velocity_ref += velocity_adj;
            if(velocity_ref > MAX_SPEED){
              velocity_ref = MAX_SPEED;
            }
            
            double x_point = x_add + target_x/(target_dis/(0.02*velocity_ref/2.24));
